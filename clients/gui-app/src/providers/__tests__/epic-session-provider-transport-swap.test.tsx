...
describe("<EpicSessionProvider /> transport swap", () => {
  ...
  it("re-opens a session that started with no live transport once one appears", async () => {
    const streams: ControlledStream[] = [];
    const seenHandles: OpenEpicStoreHandle[] = [];
    __setEpicStreamClientFactoryForTests((_epicId, _callbacks) => {
      const stream: ControlledStream = { closeCount: 0 };
    ...
    // Mount while no transport is live (host transiently not-ready): the
    // session starts on a pending stream bound to null.
    clientHolder.current = null;
    const view = render(
      <EpicSessionProvider epicId="epic-pending" tabId="epic-pending">
        <HandleProbe
          onHandle={(handle) => {
            seenHandles.push(handle);
          }}
    ...
    // A live transport arrives - the reused session must re-open on it.
    act(() => {
      clientHolder.current = {};
      view.rerender(
        <EpicSessionProvider epicId="epic-pending" tabId="epic-pending">
