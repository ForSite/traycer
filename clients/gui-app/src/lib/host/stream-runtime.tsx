...
export function HostStreamProvider(props: HostStreamProviderProps): ReactNode {
  ...
  // Mirror the live client into the module-level snapshot during render. The
  // open-epic store's stream factory (cached in a registry that outlives any
  // EpicSessionProvider instance) reads this to resolve the CURRENT transport.
  // It MUST be set in render, not an effect: this provider is an ancestor of the
  // routed tree, and an ancestor's render runs before every descendant effect in
  // the same commit - so a session re-subscribing from a child effect (e.g. on
  // the remount after a host restart) reads the new client, never the closed one.
  setLiveHostStreamClient(value?.wsStreamClient ?? null);
  useCloseWsStreamClientOnReplace(value?.wsStreamClient ?? null);
  useStreamWakeReconnect(value?.wsStreamClient ?? null);
  // Clear the module mirror on teardown (app quit / sign-out) so a stale,
  // about-to-be-closed client can't linger in the snapshot for a late factory
  // call to subscribe on - it would resolve to an inert pending stream instead.
  useEffect(() => () => setLiveHostStreamClient(null), []);
